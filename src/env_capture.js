...
export class EnvironmentCapture {
  ...
  constructor(regl, resources) {
    ...
    const faces = [0, 1, 2, 3, 4, 5].map((side_idx) =>
      this.get_resource_checked(`cube_side_${side_idx}.png`)
    );
    ...
    this.flattened_cubemap_pipeline = regl({
      ...
      uniforms: {
        ...
        preview_rect_scale: ({ viewportWidth, viewportHeight }) => {
          ...
          return [width_in_viewport_units / 3, heigh_in_viewport_units / 2];
        },
  ...
  cube_camera_view(side_idx, center, mat_view_camera) {
    const center_position_view = vec3.transformMat4(
      [0, 0, 0],
      center,
      mat_view_camera
    );
  ...
  capture_scene_cubemap(
    ...
    for (let side_idx = 0; side_idx < 6; side_idx++) {
      ...
      frame_info_with_cubemap.mat_view = this.cube_camera_view(
        side_idx,
        capture_center,
        scene_mat_view
      );
