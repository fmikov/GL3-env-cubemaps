...
class SysRenderMeshes {
  ...
  render(frame_info, scene_info) {
    ...
    for (const actor of scene_info.actors) {
      ...
      const { mat_model_view, mat_mvp, mat_normals_to_view } =
        this.make_transformation_matrices(frame_info, actor);
  ...
  render(frame_info, scene_info) {
    ...
    for (const light_actor of scene_info.actors) {
      ...
      this.env_capture.capture_scene_cubemap(
        frame_info,
        scene_info,
        light_actor.translation,
        (frame_info, scene_info) => {
          this.render_shadowmap(frame_info, scene_info);
        }
      );

      const light_position_cam = vec3.transformMat4(
        [0, 0, 0],
        light_actor.translation,
        mat_view
      );
      const light_color = vec3.scale(
        [0, 0, 0],
        light_actor.light.color,
        light_actor.light.intensity
      );

      this.render_light_contributions(
        frame_info,
        scene_info,
        light_position_cam,
        light_color
